...
// --- BANCO DE DADOS (APENAS O ESSENCIAL) ---
db.serialize(() => {
...
<head>
    ...
    <style>
        ...
        .card { background: rgba(255,255,255,0.6); backdrop-filter: blur(10px); border: 2px solid #bae6fd; border-radius: 20px; padding: 20px; margin-bottom: 15px; }
...
<body>
    ...
    <script>
        ...
        async function comprar() {
            ...
            const d = await res.json();
            if(d.qrcode) document.getElementById('pix-area').innerHTML = '<b>Pague o PIX:</b><br><img src="data:image/png;base64,'+d.qrcode+'" width="180"><br><small>'+d.payload+'</small>';
        }
        async function life(on) {
            const v = document.getElementById('vid');
            if(on) { const s = await navigator.mediaDevices.getUserMedia({video:true, audio:true}); v.srcObject = s; }
            else { if(v.srcObject) v.srcObject.getTracks().forEach(t => t.stop()); v.srcObject = null; }
        }
    </script>
</body>
</html>
...
app.post('/pix', (req, res) => {
    db.get("SELECT asaas_id, id FROM users WHERE username = ?", [req.body.u], async (err, user) => {
        try {
            const pay = await axios.post(`${ASAAS_URL}/payments`, { customer: user.asaas_id, billingType: "PIX", value: req.body.val, dueDate: new Date().toISOString().split('T'), externalReference: user.id.toString() }, { headers: { access_token: ASAAS_KEY } });
...
app.post('/webhook', (req, res) => {
    if (req.body.event === "PAYMENT_RECEIVED") {
        ...
        db.run("UPDATE users SET blues = blues + ? WHERE id = ?", [val * 0.85, userId]);
        db.get("SELECT pai_id FROM users WHERE id = ?", [userId], (err, user) => {
            if (user && user.pai_id) db.run("UPDATE users SET blues = blues + ? WHERE id = ?", [val * 0.05, user.pai_id]);
        });
    }
    res.sendStatus(200);
});
