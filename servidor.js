...
<head>
    ...
    <style>
        ...
        .card { background: rgba(255,255,255,0.6); backdrop-filter: blur(10px); border-radius: 20px; border: 2px solid #bae6fd; padding: 20px; margin-bottom: 15px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); }
        ...
        video { width: 100%; border-radius: 15px; background: #000; margin-top: 10px; border: 3px solid #7dd3fc; }
    ...
    <script>
        ...
        async function auth(rota) {
            ...
            if(res.ok) {
                ...
                document.getElementById('perfil-txt').innerHTML = (d.role === 'master' ? '<span class="estrela">⭐ MASTER</span>' : '') + '<h2>' + d.username.toUpperCase() + '</h2><p>Saldo: ' + d.blues.toFixed(2) + ' Blues</p>';
        ...
        async function comprar() {
            ...
            const d = await res.json();
            if(d.qrcode) {
                document.getElementById('pix-area').innerHTML = '<b>Escaneie o PIX:</b><br><img src="data:image/png;base64,'+d.qrcode+'" width="200"><br><small style="word-break:break-all">Copia e Cola: '+d.payload+'</small>';
            }
        }
...
app.post('/register', async (req, res) => {
    try {
        ...
        const resdb = db.prepare("INSERT INTO users (username, password, asaas_id) VALUES (?, ?, ?)").run(req.body.u, hash, cli.data.id);
        if(resdb.lastInsertRowid === 1) db.prepare("UPDATE users SET role = 'master' WHERE id = 1").run();
        res.json({ ok: true });
    } catch (e) { res.status(400).json({ error: "Erro no registro" }); }
});

app.post('/login', (req, res) => {
    const user = db.prepare("SELECT * FROM users WHERE username = ?").get(req.body.u);
...
app.post('/pix', async (req, res) => {
    try {
        const user = db.prepare("SELECT asaas_id FROM users WHERE username = ?").get(req.body.u);
    ...
    } catch (e) { res.status(500).json({ error: "Erro no PIX" }); }
});
